...
def duplicate_frequency(my_input):
    freq = 0
    freq_list = [0]
    for i in my_input:
        freq += i
    ...
    return first_duplicate(freq_list)


def part_2(my_input):
    frequency_found = False
    counter = 1
    while not frequency_found or counter <= 100000:
        list_try = []
        for i in range(counter):
            list_try = list_try + my_input
        res = duplicate_frequency(list_try)
        if res is not None:
            return res
        else:
            counter += 1
...
print("Final frequency is : {}".format(part_1(p_input)))

print("First duplicate frequency is : {}".format(part_2(p_input)))
